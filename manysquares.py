# E:\ppython\app\python.exe manysquares.py
...
def setsmalltiles():
    ...
    for x in range(int(b1awide)):
        for y in range(int(b1ahigh)):
            pygame.draw.rect(windowSurface, GREEN, (5+x*(SMALLSQUARESIDE+SMALLSQUAREGAP), 5+y*(SMALLSQUARESIDE+SMALLSQUAREGAP), SMALLSQUARESIDE+random.randint(-1,1), SMALLSQUARESIDE+random.randint(-1,1)))
