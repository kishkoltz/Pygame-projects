# E:\ppython\app\python.exe aimsquares2.py
...
def getEatables():
    global eatables
    for x in range(40):
        eatables.append(random.randint(0, len(grid)))
def drawEatables():
    for e in range(len(eatables)):
        pygame.draw.rect(windowSurface, GREEN,
        (grid[eatables[e]][0], grid[eatables[e]][1], SMALLSQUARESIDE, SMALLSQUARESIDE))
def eatableEaten():
    for e in range(len(eatables)):
        if grid[eatables[e]][0] >= squareField[0]\
            and grid[eatables[e]][1] >= squareField[1]\
            and grid[eatables[e]][2] <= squareField[2]\
            and grid[eatables[e]][3] <= squareField[3]:
                pygame.draw.rect(windowSurface, BLUE,
                (grid[eatables[e]][0], grid[eatables[e]][1],
